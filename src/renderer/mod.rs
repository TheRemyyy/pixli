...
pub use types::{GraphicsSettings, GpuMesh, GpuVertex, UnlitMesh, UnlitMeshRef, Uniforms, UnlitVertex};
...
impl Renderer {
    pub fn new() -> Self {
        ...
        Self {
            ...
            unlit_uniform_scratch: Vec::with_capacity(MAX_UNLIT_DRAWS * UNLIT_INSTANCE_SIZE as usize),
            ...
            shadow_entity_scratch: Vec::with_capacity(MAX_LIT_DRAWS * SHADOW_ENTITY_STRIDE as usize),
    ...
    pub fn upload_unlit_mesh(&mut self, mesh: &UnlitMesh) -> u64 {
        ...
        self.unlit_mesh_cache.insert(id, GpuMesh {
            vertex_buffer,
    ...
    pub fn init(&mut self, device: Arc<wgpu::Device>, queue: Arc<wgpu::Queue>, format: wgpu::TextureFormat, width: u32, height: u32) {
        self.device = Some(device.clone());
        self.queue = Some(queue.clone());
        self.surface_format = format;

        let r = init::create_gpu_resources(device.as_ref(), queue.as_ref(), format, self.msaa_samples);
    ...
    pub fn resize(&mut self, width: u32, height: u32) {
        ...
        let depth_texture = device.create_texture(&wgpu::TextureDescriptor {
            ...
            size: wgpu::Extent3d { width: internal_width, height: internal_height, depth_or_array_layers: 1 },
            mip_level_count: 1,
        ...
        if let (Some(depth_ssao_view_ref), Some(ssao_pipe), Some(params_buf), Some(sampler), Some(blur_pipe), Some(blur_params_buffer), Some(ssao_tex_view), Some(ssao_blur_view_ref)) = (
            ...
            let ssao_bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
                layout: &ssao_bgl,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(depth_ssao_view_ref) },
                    wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(sampler) },
                    wgpu::BindGroupEntry { binding: 2, resource: params_buf.as_entire_binding() },
                ],
                label: Some("ssao_bg"),
            ...
            let ssao_blur_bg_in = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(ssao_tex_view) },
                    wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(sampler) },
                    wgpu::BindGroupEntry { binding: 2, resource: blur_params_buffer.as_entire_binding() },
                ],
            ...
            let ssao_blur_bg_out = device.create_bind_group(&wgpu::BindGroupDescriptor {
                layout: &ssao_blur_bgl,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(ssao_blur_view_ref) },
                    wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(sampler) },
                    wgpu::BindGroupEntry { binding: 2, resource: blur_params_buffer.as_entire_binding() },
                ],
        ...
        if let (Some(layout), Some(sampler), Some(sv), Some(bloom_view), Some(ssao_view)) = (
            ...
            self.post_bind_group = Some(device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(ssao_view) },
    ...
    pub fn upload_mesh(&mut self, mesh: &Mesh) -> u64 {
        ...
        let vertices: Vec<GpuVertex> = mesh.vertices.iter().map(|v| {
            GpuVertex {
        ...
        self.mesh_cache.insert(id, GpuMesh {
            vertex_buffer,
    ...
    fn build_unlit_batches(
        ...
        let unlit_entities: Vec<_> = world
            ...
            .collect();
        let mut by_mesh: std::collections::HashMap<u64, Vec<Entity>> = std::collections::HashMap::new();
        for &e in &unlit_entities {
            let Some(unlit_ref) = world.get::<UnlitMeshRef>(e) else { continue };
        ...
        if !entity_order.is_empty() {
            ...
            self.unlit_uniform_scratch.resize(n * UNLIT_INSTANCE_SIZE as usize, 0);
            for (idx, &entity) in entity_order.iter().enumerate() {
                ...
                self.unlit_uniform_scratch[slot_start..slot_start + 64].copy_from_slice(bytemuck::bytes_of(&model_arr));
                self.unlit_uniform_scratch[slot_start + 64..slot_start + 128].copy_from_slice(bytemuck::bytes_of(&mvp_arr));
            }
        }
    ...
    pub fn render(&mut self, world: &World, view: &wgpu::TextureView) {
        ...
        let Some(msaa_texture) = &self.msaa_texture else { return };
        let Some(unlit_pipeline) = &self.unlit_pipeline else { return };
        let Some(unlit_bind_group) = &self.unlit_bind_group else { return };
        let Some(unlit_storage_buffer) = &self.unlit_storage_buffer else { return };
        let Some(unlit_batch_start_buffer) = &self.unlit_batch_start_buffer else { return };
        let Some(unlit_scene_buffer) = &self.unlit_scene_buffer else { return };
        let Some(sky_pipeline) = &self.sky_pipeline else { return };
        let Some(sky_bind_group) = &self.sky_bind_group else { return };
        let Some(sky_vertex_buffer) = &self.sky_vertex_buffer else { return };
        let Some(sky_uniform_buffer) = &self.sky_uniform_buffer else { return };
        let Some(scene_texture_view) = &self.scene_texture_view else { return };
        let Some(post_pipeline) = &self.post_pipeline else { return };
        let Some(post_bind_group) = &self.post_bind_group else { return };
        let Some(post_vertex_buffer) = &self.post_vertex_buffer else { return };
        ...
        let (light_dir, light_color, light_intensity) = if let Some(ref light) = self.directional_light {
        ...
        if self.graphics.enable_ssao {
            if let (Some(depth_ssao_view), Some(depth_lit_pipe), Some(depth_unlit_pipe), Some(depth_lit_bg), Some(depth_unlit_bg), Some(depth_entity_buf)) = (
                self.depth_ssao_view.as_ref(),
        ...
        run_post_pass(&mut encoder, view, post_pipeline, post_bind_group, post_vertex_buffer);
    ...
    fn recreate_pipelines(&mut self) {
        if let (Some(device), Some(layout)) = (
            self.device.as_ref(),
            self.lit_pipeline_layout.as_ref(),
        ) {
            let (pipeline, unlit_pipeline, sky_pipeline) =
                init::recreate_msaa_pipelines(device, self.surface_format, self.msaa_samples, layout);
...
fn run_depth_prepass_ssao(
    ...
    for (i, entity) in lit_entities.iter().enumerate() {
        ...
        depth_ssao_entity_scratch[slot..slot + 64].copy_from_slice(bytemuck::bytes_of(&mvp.to_cols_array()));
    }
    queue.write_buffer(
        depth_entity_buf,
        0,
    ...
    if !entity_order.is_empty() {
        ...
        for (batch_idx, (mesh_id, _start, instance_count)) in batches.iter().enumerate() {
            depth_pass.set_bind_group(0, depth_unlit_bg, &[((batch_idx as u64) * UNLIT_BATCH_START_STRIDE) as u32]);
    ...
    if !lit_entities.is_empty() {
        ...
        for (i, entity) in lit_entities.iter().enumerate() {
            let Some(mesh) = world.get::<Mesh>(*entity) else { continue };
...
fn run_shadow_pass(
    ...
    if enable_shadows && !lit_entities.is_empty() && light.light_type == LightType::Directional {
        ...
        let up = if light_dir.y.abs() > 0.99 { Vec3::RIGHT } else { Vec3::UP };
        ...
        for (i, entity) in lit_entities.iter().enumerate() {
            ...
            let slot = i * SHADOW_ENTITY_STRIDE as usize;
            shadow_entity_scratch[slot..slot + 64].copy_from_slice(bytemuck::bytes_of(&model.to_cols_array()));
            shadow_entity_scratch[slot + 64..slot + 128].copy_from_slice(bytemuck::bytes_of(&light_mvp.to_cols_array()));
        }
        queue.write_buffer(
            shadow_entity_buffer,
            0,
        ...
        for (i, entity) in lit_entities.iter().enumerate() {
            let Some(mesh) = world.get::<Mesh>(*entity) else { continue };
            shadow_pass.set_bind_group(0, shadow_bind_group, &[(i as u64 * SHADOW_ENTITY_STRIDE) as u32]);
            if let Some(gpu_mesh) = mesh_cache.get(&mesh.id()) {
...
fn run_bloom(
    ...
    if enable_bloom {
        if let (Some(extract_pipe), Some(blur_pipe), Some((bg_extract, bg_blur_a, bg_blur_b)), Some(params_buf), Some(bloom_a), Some(bloom_b), Some(pvb)) = (
            extract_pipe,
            blur_pipe,
            bind_groups,
            params_buf,
            ...
            queue.write_buffer(params_buf, 0, bytemuck::bytes_of(&BlurParams { texel_size: texel, is_horizontal: 1, _pad: 0 }));
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            ...
            queue.write_buffer(params_buf, 0, bytemuck::bytes_of(&BlurParams { texel_size: texel, is_horizontal: 0, _pad: 0 }));
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
...
fn run_ssao_pass(
    ...
    if enable_ssao {
        if let (Some(ssao_pipe), Some(ssao_params_buf), Some((ssao_bg, ssao_blur_in, ssao_blur_out)), Some(blur_pipe), Some(blur_params_buf), Some(ssao_view), Some(ssao_blur_view), Some(pvb)) = (
            ssao_pipe,
            ssao_params_buf,
            ssao_bind_groups,
            blur_pipe,
            ...
            queue.write_buffer(blur_params_buf, 0, bytemuck::bytes_of(&BlurParams { texel_size: texel, is_horizontal: 1, _pad: 0 }));
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            ...
            queue.write_buffer(blur_params_buf, 0, bytemuck::bytes_of(&BlurParams { texel_size: texel, is_horizontal: 0, _pad: 0 }));
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
...
fn run_main_pass(
    ...
    if !lit_entities.is_empty() {
        ...
        for (i, entity) in lit_entities.iter().enumerate() {
            let Some(transform) = world.get::<Transform>(*entity) else { continue };
            let Some(_) = world.get::<Mesh>(*entity) else { continue };
