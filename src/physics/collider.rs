...
impl Collider {
    ...
    pub fn box_collider(size: Vec3) -> Self {
        Self {
            shape: ColliderShape::Box { half_extents: size * 0.5 },
            offset: Vec3::ZERO,
            is_trigger: false,
        }
    }
    ...
    pub fn collision_info(&self, other: &Collider, pos_a: Vec3, pos_b: Vec3) -> Option<(Vec3, Vec3, f32)> {
        let pos_a = pos_a + self.offset;
        let pos_b = pos_b + other.offset;
        ...
        match (&self.shape, &other.shape) {
            ...
            (ColliderShape::Sphere { radius }, ColliderShape::Box { half_extents }) => {
                Self::aabb_sphere_info(pos_b, *half_extents, pos_a, *radius).map(|(p, n, d)| (p, -n, d))
            }
            _ => None,
        }
    }
    ...
    pub fn raycast(&self, origin: Vec3, direction: Vec3, collider_pos: Vec3) -> Option<(Vec3, f32, Vec3)> {
        let pos = collider_pos + self.offset;

        match &self.shape {
            ColliderShape::Sphere { radius } => {
                Self::ray_sphere(origin, direction, pos, *radius)
            }
    ...
    fn aabb_aabb(pos_a: Vec3, half_a: Vec3, pos_b: Vec3, half_b: Vec3) -> bool {
        ...
        (pos_a.y - half_a.y <= pos_b.y + half_b.y && pos_a.y + half_a.y >= pos_b.y - half_b.y) &&
        (pos_a.z - half_a.z <= pos_b.z + half_b.z && pos_a.z + half_a.z >= pos_b.z - half_b.z)
    }
    ...
    fn aabb_sphere(box_pos: Vec3, half_extents: Vec3, sphere_pos: Vec3, radius: f32) -> bool {
        let closest = Vec3::new(
            sphere_pos.x.clamp(box_pos.x - half_extents.x, box_pos.x + half_extents.x),
            sphere_pos.y.clamp(box_pos.y - half_extents.y, box_pos.y + half_extents.y),
            sphere_pos.z.clamp(box_pos.z - half_extents.z, box_pos.z + half_extents.z),
        );
        (closest - sphere_pos).length_squared() <= radius * radius
    }

    fn aabb_aabb_info(pos_a: Vec3, half_a: Vec3, pos_b: Vec3, half_b: Vec3) -> Option<(Vec3, Vec3, f32)> {
        ...
        let (normal, penetration) = if overlap_x < overlap_y && overlap_x < overlap_z {
            (Vec3::new(if pos_a.x < pos_b.x { -1.0 } else { 1.0 }, 0.0, 0.0), overlap_x)
        } else if overlap_y < overlap_z {
            (Vec3::new(0.0, if pos_a.y < pos_b.y { -1.0 } else { 1.0 }, 0.0), overlap_y)
        } else {
            (Vec3::new(0.0, 0.0, if pos_a.z < pos_b.z { -1.0 } else { 1.0 }), overlap_z)
        };
    ...
    fn sphere_sphere_info(pos_a: Vec3, radius_a: f32, pos_b: Vec3, radius_b: f32) -> Option<(Vec3, Vec3, f32)> {
    ...
    fn aabb_sphere_info(box_pos: Vec3, half_extents: Vec3, sphere_pos: Vec3, radius: f32) -> Option<(Vec3, Vec3, f32)> {
        let closest = Vec3::new(
            sphere_pos.x.clamp(box_pos.x - half_extents.x, box_pos.x + half_extents.x),
            sphere_pos.y.clamp(box_pos.y - half_extents.y, box_pos.y + half_extents.y),
            sphere_pos.z.clamp(box_pos.z - half_extents.z, box_pos.z + half_extents.z),
    ...
    fn ray_sphere(origin: Vec3, direction: Vec3, center: Vec3, radius: f32) -> Option<(Vec3, f32, Vec3)> {
    ...
    fn ray_aabb(origin: Vec3, direction: Vec3, center: Vec3, half_extents: Vec3) -> Option<(Vec3, f32, Vec3)> {
