...
impl Collider {
    ...
    pub fn collision_info(
        ...
        match (&self.shape, &other.shape) {
            ...
            ) => Self::capsule_sphere_info(pos_b, *ra, *ha, pos_a, *rb).map(|(p, n, d)| (p, -n, d)),
            (
                ColliderShape::Capsule {
                    radius: ra,
                    height: ha,
    ...
    fn capsule_segment(pos: Vec3, radius: f32, height: f32) -> (Vec3, Vec3) {
        ...
        (
            pos - Vec3::new(0.0, half_axis, 0.0),
            pos + Vec3::new(0.0, half_axis, 0.0),
        )
    }
    ...
    fn capsule_capsule(
        ...
        let (closest_a, closest_b) =
            Self::closest_point_segment_to_segment(low_a, high_a, low_b, high_b);
        (closest_b - closest_a).length_squared() <= (radius_a + radius_b).powi(2)
    }

    fn capsule_capsule_info(
        pos_a: Vec3,
        radius_a: f32,
        height_a: f32,
        pos_b: Vec3,
